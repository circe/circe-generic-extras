...
class ConfiguredSemiautoDerivedSuite extends CirceSuite {
  ...
  // this fails because `circe-core` Scala 3 derivation calls the `transformName` functions for each encode / decode invocation.
  // to be fixed in `circe-core` ???
  // see `memberNameCache`
  ...
  //     println("transformMemberNamesCallCount" -> transformMemberNamesCallCount)
  //     println("transformConstructorCallCount" -> transformConstructorCallCount)
  //     assert(transformMemberNamesCallCount === fieldCount * 2) // failure: 12 in second call
